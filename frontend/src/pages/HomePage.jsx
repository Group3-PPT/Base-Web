...
const DISTRICTS = ["", "Quận 1", "Quận 2", "Quận 3", "Quận 4", "Quận 5", "Quận 6", "Quận 7", "Quận 8", "Quận 9", "Quận 10", "Quận 11", "Quận 12", "Bình Thạnh", "Gò Vấp", "Phú Nhuận", "Tân Bình", "Tân Phú", "Bình Tân", "Thủ Đức"];
...
export default function HomePage() {
  ...
  const [filters, setFilters] = useState({
    search: "", district: "", listing_type: "", status: "",
    min_price: "", max_price: "", min_area: "", max_area: "", page: 1, limit: 20,
  });
  ...
  return (
    <div>
      ...
      {/* Filters */}
      <div className="bg-white rounded-xl shadow-sm border border-gray-200/60 p-5 mb-6">
        <h3 className="text-sm font-bold text-gray-800 mb-4">Bộ lọc</h3>
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-3">
          <div>
            ...
            <select value={filters.district} onChange={(e) => updateFilter("district", e.target.value)}
              className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm bg-gray-50 focus:bg-white focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none">
          ...
          <div>
            ...
            <input type="number" placeholder="Từ" value={filters.min_price} onChange={(e) => updateFilter("min_price", e.target.value)}
              className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm bg-gray-50 focus:bg-white focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none" />
          </div>
          <div>
            <label className="block text-xs text-gray-500 mb-1">Giá đến (VND)</label>
            <input type="number" placeholder="Đến" value={filters.max_price} onChange={(e) => updateFilter("max_price", e.target.value)}
              className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm bg-gray-50 focus:bg-white focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none" />
          </div>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div>
            <label className="block text-xs text-gray-500 mb-1">Diện tích từ (m²)</label>
            <input type="number" placeholder="Từ" value={filters.min_area} onChange={(e) => updateFilter("min_area", e.target.value)}
              className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm bg-gray-50 focus:bg-white focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none" />
          </div>
          <div>
            <label className="block text-xs text-gray-500 mb-1">Diện tích đến (m²)</label>
            <input type="number" placeholder="Đến" value={filters.max_area} onChange={(e) => updateFilter("max_area", e.target.value)}
              className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm bg-gray-50 focus:bg-white focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none" />
          </div>
          <div className="flex items-end gap-2 col-span-2 md:col-span-2">
            <button onClick={() => setFilters({ search: "", district: "", listing_type: "", status: "", min_price: "", max_price: "", min_area: "", max_area: "", page: 1, limit: 20 })}
              className="px-4 py-2 border border-gray-200 rounded-lg text-sm text-gray-600 hover:bg-gray-50 transition-colors">
              Xoá bộ lọc
            </button>
            <button onClick={fetchData} className="px-6 py-2 bg-blue-600 text-white rounded-lg text-sm font-semibold hover:bg-blue-700 transition-colors shadow-sm">
              Áp dụng
            </button>
          </div>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-white rounded-xl border border-gray-200/60">
          ...
          <p className="text-gray-300 text-xs mt-1">Thử thay đổi bộ lọc hoặc từ khóa</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
        ...
        <div className="flex items-center justify-center gap-1.5 mt-8">
          ...
          {Array.from({ length: pagination.totalPages }, (_, i) => i + 1).map((page) => (
            <button key={page} onClick={() => setFilters((f) => ({ ...f, page }))}
              className={`w-9 h-9 rounded-lg text-sm font-medium transition-colors ${page === filters.page ? "bg-blue-600 text-white shadow-sm" : "bg-white border border-gray-200 text-gray-600 hover:bg-gray-50"}`}>
              {page}
            </button>
          ))}
